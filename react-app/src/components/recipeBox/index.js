	...
	return (
		<>
			<section className='recipeBoxContainer'>
				...
				<NavLink exact to='/recipebox/new'>
					<button id='createNewRecipeButton'>Create New Recipe</button>
				</NavLink>
					...
					<ul className='recipesInBox'>
						{userRecipeBox.map(({ name, id, user_id }) => (
							<li id='mappedRecipesInBox' key={id}>
								<NavLink exact to={`/recipes/${id}`}>
									{name}
								</NavLink>
								<section className='recipeBoxButtons'>
									<OpenModalButton
										buttonText='Add to Meal Planner'
										onItemClick={closeMenu}
										className='recipeBoxAddTo'
										modalComponent={
											<MealPlannerModal recipeId={id} userId={userId} />
										}
									/>
									{user_id === sessionUser.id && (
										<>
											<NavLink exact to={`/recipebox/update/${id}`}>
												<button className='rbUpdate'>Update</button>
											</NavLink>
											<button
												className='rbDelete'
